...
function contractMismatchResponse() {
  return NextResponse.json(
    { error: "Backend contract mismatch for the prescription dispense route." },
    { status: 502 }
  );
}
...
export async function POST(
  ...
  const auth = requirePermission(request, "prescription.dispense");
  if (auth.error) {
    return auth.error;
  }
  ...
  const backend = await callBackendRoute(request, `/v1/prescriptions/${id.value}/dispense`, {
  ...
  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to dispense prescription.");
  }
