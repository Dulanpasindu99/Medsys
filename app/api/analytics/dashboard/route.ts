...
function contractMismatchResponse() {
  return NextResponse.json(
    { error: "Backend contract mismatch for the analytics dashboard route." },
    { status: 502 }
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const backend = await callBackendRoute(request, "/v1/analytics/dashboard");
  if (!backend.ok) {
    return backend.response;
  }

  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to load analytics dashboard.");
  }
