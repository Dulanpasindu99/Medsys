...
function contractMismatchResponse() {
  return NextResponse.json(
    { error: "Backend contract mismatch for the inventory reports route." },
    { status: 502 }
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const backend = await callBackendRoute(request, "/v1/inventory/reports", {
    includeSearch: true,
  });
  if (!backend.ok) {
  ...
  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to load inventory reports.");
  }
