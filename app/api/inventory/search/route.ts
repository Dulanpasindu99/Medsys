...
function contractMismatchResponse() {
  return NextResponse.json(
    { error: "Backend contract mismatch for the inventory search route." },
    { status: 502 }
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const backend = await callBackendRoute(request, "/v1/inventory/search", {
    includeSearch: true,
  });
  if (!backend.ok) {
  ...
  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to search inventory.");
  }
