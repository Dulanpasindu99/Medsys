...
function contractMismatchResponse() {
  return NextResponse.json(
    { error: "Backend contract mismatch for the encounters route." },
    { status: 502 }
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const backend = await callBackendRoute(request, "/v1/encounters", {
    includeSearch: false,
  });
  if (!backend.ok) {
  ...
  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to load encounters.");
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!backend.response.ok) {
    return toFrontendErrorResponse(backend.response, "Unable to save encounter.");
  }
