...
type DoctorSidebarProps = {
  ...
  onClearPatientActions?: () => void;
  canClearPatientActions?: boolean;
...
function SidebarSection({
  ...
  return (
    <section
      className={`border-t border-slate-100/90 p-2.5 first:border-t-0 ${className}`}
    >
          ...
          <div className="min-h-0 flex-1 overflow-visible lg:overflow-y-auto">
            ...
            >
              {selectedPatientProfileId ? (
                ...
                <p className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-[13px] font-semibold text-amber-800">
                ...
                <div className="space-y-2">
                  ...
                  {assistantRegistrationHref ? (
                    <a
                      ...
                      className="inline-flex min-h-9 w-full items-center justify-center rounded-xl bg-gradient-to-r from-sky-500 via-sky-600 to-blue-600 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-white shadow-[0_10px_20px_rgba(59,130,246,0.24)] transition hover:-translate-y-0.5"
                    >
                    ...
                    <div className="space-y-1.5">
                      <button
                        type="button"
                        disabled
                        className="inline-flex min-h-9 w-full items-center justify-center rounded-xl border border-slate-200 bg-slate-100 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500"
                      >
                        Create Patient & Start Visit
                      </button>
              ...
              {showPatientEditors ? (
                <div className="space-y-2">
                  {latestVitalsObservedAt ? (
                    <p className="rounded-2xl bg-slate-50 px-3 py-2 text-[12px] font-semibold text-slate-600 ring-1 ring-slate-100">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {vitalFieldMeta.map(({ key, label, placeholder }) => (
                      <label
                        key={key}
                        className="flex min-h-[92px] flex-col rounded-2xl border border-white/70 bg-white/80 p-2.5 shadow-[0_8px_22px_rgba(14,165,233,0.1)] ring-1 ring-sky-50"
                      >
                        <div className="flex min-h-[24px] items-start justify-between gap-2">
                          ...
                          {key === "temperature" ? (
                            ...
                            <span className="block h-6 w-[48px] opacity-0" aria-hidden="true" />
                          )}
                        </div>
                        <input
                          value={
                          ...
                          disabled={!canEditVitals}
                          className="mt-1.5 h-9 w-full rounded-xl border border-slate-200 bg-slate-50 px-2.5 text-[13px] font-semibold text-slate-900 outline-none transition placeholder:text-[11px] placeholder:font-medium placeholder:text-slate-400 focus:border-sky-300 focus:bg-white focus:ring-2 focus:ring-sky-100 disabled:cursor-not-allowed disabled:opacity-70"
              ...
              <div className="space-y-2">
                {patientAllergies.length ? (
                  patientAllergies.map((allergy) => (
                    <div
                      key={allergy.name}
                      className={`flex items-center justify-between rounded-2xl bg-white/90 px-3 py-2 ring-1 shadow-[0_10px_22px_rgba(15,23,42,0.08)] ${
                    ...
                    >
                      ...
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-[0.16em] ${allergy.pill}`}
                      >
                        <span
                ...
                {consultationAllergies.length ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-2.5">
                    <p className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-700">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {consultationAllergies.map((allergy) => (
                        <button
                          ...
                          className="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200"
                        >
                ...
                {showPatientEditors ? (
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-2.5 ring-1 ring-slate-100">
                    <div className="grid items-stretch gap-2 md:grid-cols-[minmax(0,1fr)_148px]">
                      <input
                        ...
                        className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-[13px] font-semibold text-slate-900 outline-none transition placeholder:text-[11px] focus:border-rose-300 focus:ring-2 focus:ring-rose-100 disabled:cursor-not-allowed disabled:opacity-70"
                    ...
                    <div className="mt-2 flex min-h-6 flex-wrap gap-2">
                      ...
                      allergyDraftName.trim().length > 0 ? (
                        <button
                          ...
                          className="inline-flex min-h-9 items-center justify-center rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500 transition hover:bg-slate-50"
                        >
                          Clear
                        </button>
                      ) : null}
