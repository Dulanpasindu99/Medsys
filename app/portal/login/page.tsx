    ...
    <main className="flex min-h-dvh items-center justify-center bg-gradient-to-b from-white via-slate-50 to-sky-50/50 px-4 py-10">
      <div className="w-full max-w-md rounded-[28px] border border-white/80 bg-white/95 p-6 shadow-[0_22px_54px_rgba(15,23,42,0.12)] ring-1 ring-slate-100 sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-black tracking-tight text-slate-900">
          {mode === "login" ? t("Patient sign in") : t("Create your account")}
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          {mode === "login"
            ? t("Access your records, prescriptions and reports.")
            : t("Sign up to manage your health records online.")}
        </p>
        ...
        <div className="mt-5 grid grid-cols-2 gap-1 rounded-full bg-slate-100 p-1 text-sm font-semibold">
          {(["login", "signup"] as const).map((value) => (
            ...
            >
              {value === "login" ? t("Sign in") : t("Sign up")}
            </button>
          ))}
        </div>

        <form className="mt-5 space-y-3" onSubmit={submit}>
          <Field label={t("Email")} type="email" value={email} onChange={setEmail} required placeholder="you@email.com" />
          {mode === "signup" ? (
            <>
              <Field label={t("Phone (optional)")} type="tel" value={phone} onChange={setPhone} placeholder="+94 7X XXX XXXX" />
              <Field label={t("NIC (optional)")} type="text" value={nic} onChange={setNic} placeholder={t("Connects your existing records")} />
            </>
          ) : null}
          <Field label={t("Password")} type="password" value={password} onChange={setPassword} required placeholder="••••••••" />
          {mode === "signup" ? (
            <Field label={t("Confirm password")} type="password" value={confirm} onChange={setConfirm} required placeholder="••••••••" />
          ) : null}
